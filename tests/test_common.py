...
def test_stripsplit():
    ...
    assert tuple(common.stripsplit("   ")) == ()
    assert tuple(common.stripsplit(" many   differnt   things ")) == ("many",
            "differnt", "things")
    assert tuple(common.stripsplit(",,,,", ",")) == ()
    assert tuple(common.stripsplit("one, two, three", ",")) == ("one",
            "two", "three")
    assert tuple(common.stripsplit("one, two, three", ",", 1)) == ("one",
            "two, three")
    assert tuple(common.stripsplit("==one==, ==two==, three  ,=four===",
            ",", 2, '= ')) == ('one', 'two', 'three  ,=four')


def test_uniques():
    """Test submodule common's uniques."""
    assert common.uniques((1, 2, 3), (2, 3, 4), (3, 4, 5) * 10) == \
                (1, 2, 3, 4, 5)
    assert common.uniques(tuple(range(20)), range(10, 30),
                target=[]) == list(range(30))
    assert common.uniques((1, 2, 3), (2, 2, 3, 4, 4, 4),
                (3, 4, 5, 3, 4, 5, 3, 4, 5), ()) == (1, 2, 3, 4, 5)
    assert common.uniques((1, 2, 3), (2, 2, 3, 4, 4, 4),
                (3, 4, 5, 3, 4, 5, 3, 4, 5),
                (), target=[]) == [1, 2, 3, 4, 5]
