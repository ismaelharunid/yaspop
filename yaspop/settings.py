...
class Setting(property):
    ...
    @classmethod
    def Factory(cls, clsname, cast, bases=None, mixins=None, tag=None):
    ...
    def __init__(self, name, default=None,
        ...
        dbg_print("Setting.__init__",
                name, default, getter, setter, deleter, doc)
        cast = self._cast
        ...
        if getter is True:
            if callable(setter):
                raise AttributeError("if {:s} getter is True then the"
                        .format(name) + " setter may only be True or False"
                        + " (or None defaulting to True")
            if default is RequiredValue:
                if setter is False:
                    raise AttributeError("if {:s} getter is True and"
                            .format(name) + " default is RequiredValue then"
                            + " the setter may only be True (or None"
                            + " defaulting to True")
                if deleter:
                    raise AttributeError("if {:s} getter is True and"
                            .format(name) + " default is RequiredValue"
                            + " then deleters are not allowed")
            ...
            if setter is True or default is RequiredValue:
                def setter_wrapper(self, val):
                    ...
                    if val is None:
                        if default is RequiredValue:
                            raise AttributeError("{:s} is a required value"
                                    .format(name))
        ...
        dbg_print("Setting.__init__ (before super)",
                name, default, getter, setter, deleter, doc)
        self._default = default
        super().__init__(getter, setter_wrapper, deleter, doc)
        dbg_print("Setting.__init__ (after super)",
                name, default, getter, setter, deleter, doc)
    ...
    def Factory(cls, name, settings, base=None, repr_=None):
        ...
        def __init__(self, initializer={}):
            for key in self._keys:
                self._settings[key] = settings[key](initializer[key]
                                        if key in initializer else None)
